  ...
  return (
    <div style={{ 
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <h3 style={{ marginTop: 0 }}>{editingId ? '✏️ Edit Student' : '➕ Add New Student'}</h3>
        ...
        <div style={{ marginTop: '15px', display: 'flex', gap: '10px' }}>
          ...
          >
            {loading ? '⏳ Processing...' : (editingId ? '✏️ Update Student' : '➕ Add Student')}
          </button>
          {editingId && (
            <button 
              type="button"
      ...
      {/* Danh sách sinh viên */}
      {students.length === 0 ? (
        <p style={{ textAlign: 'center', color: '#888', padding: '40px' }}>📭 No students found</p>
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{
            ...
            <tbody>
              {students.map((s) => (
                <tr key={s._id || s.id || s.studentId} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '10px' }}><strong>{s.studentId}</strong></td>
